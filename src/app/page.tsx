...
function VoterSheet({voter,token,close}:{voter:Voter;token:string;close:()=>void}) {
  const [phone,setPhone]=useState(""); const [status,setStatus]=useState("sin_contactar"); const [notes,setNotes]=useState(""); const [roles,setRoles]=useState<string[]>([]); const [savedRoles,setSavedRoles]=useState<string[]>([]); const [saved,setSaved]=useState(""); const [accessCode,setAccessCode]=useState("");
  ...
  return <div className="sheet-backdrop" onClick={close}><section className="voter-sheet" onClick={e=>e.stopPropagation()}><div className="sheet-grab"/><div className="sheet-title"><div><small>FICHA DEL VOTANTE</small><h2>{voter.apellido_nombre}</h2></div><button onClick={close}>×</button></div><dl><div><dt>DNI</dt><dd>{voter.dni}</dd></div><div><dt>Domicilio</dt><dd>{voter.domicilio||"Sin dato"}</dd></div><div><dt>Circuito</dt><dd>{voter.circuito_nombre||voter.circuito}</dd></div><div><dt>Mesa / Orden</dt><dd>{voter.mesa} / {voter.orden??"-"}</dd></div></dl><div className="profile-form"><label>Teléfono<input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="381 000 0000"/></label><label>Estado<select value={status} onChange={e=>setStatus(e.target.value)}><option value="sin_contactar">Sin contactar</option><option value="contactado">Contactado</option><option value="confirmado">Confirmado</option><option value="no_contactar">No contactar</option></select></label><fieldset><legend>Roles en el equipo</legend><div className="role-checks">{electoralRoles.map(([role,label])=><label key={role}><input type="checkbox" checked={roles.includes(role)} onChange={()=>toggleRole(role)}/><span>{label}</span></label>)}</div></fieldset>{roles.includes("movilizador")&&<div className="mobilizer-access"><b>Acceso externo</b><button type="button" onClick={generateMobilizerAccess}>GENERAR CÓDIGO MOVILIZADOR</button>{accessCode&&<code>{accessCode}</code>}</div>}<label>Observaciones<textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={3}/></label><button onClick={save}>GUARDAR CAMBIOS</button>{saved&&<p>{saved}</p>}</div></section></div>;
}
